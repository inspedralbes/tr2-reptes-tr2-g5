...
:variant="state.subFilter === 'TOTS' ? 'flat' : 'outlined'"

:color="state.subFilter === 'TOTS' ? 'black' : 'grey-darken-1'"

@click="state.subFilter = 'TOTS'"

class="mr-2"

>Totes ({{ peticions.filter(p => p.estat !== 'REBUTJADA').length }})</v-btn>
...
:variant="state.subFilter === 'PENDENTS' ? 'flat' : 'outlined'"

:color="state.subFilter === 'PENDENTS' ? 'orange-darken-3' : 'grey-darken-1'"

@click="state.subFilter = 'PENDENTS'"

prepend-icon="mdi-clock-outline"

class="mr-2"

>Per Acceptar ({{ peticions.filter(p => p.estat === 'PENDENT').length }})</v-btn>
...
:variant="state.subFilter === 'ASSIGNATS' ? 'flat' : 'outlined'"

:color="state.subFilter === 'ASSIGNATS' ? 'teal-darken-2' : 'grey-darken-1'"

@click="state.subFilter = 'ASSIGNATS'"

prepend-icon="mdi-check-all"

>Ja Assignades ({{ peticions.filter(p => p.estat === 'ASSIGNAT').length }})</v-btn>
...
<v-chip size="x-small" :color="colors[p.estat] || 'grey'" class="text-white font-weight-bold" variant="flat">
...
<p class="white--text font-weight-bold">{{ state.selected?.tallerId?.titol || 'Pendent' }}</p>

<v-chip v-if="state.selected?.tallerId?.modalitat" size="x-small" color="blue-lighten-1" variant="flat" class="mt-1">

{{ state.selected.tallerId.modalitat }}
...
<p class="text-body-1 comment-text">{{ state.selected?.comentaris }}</p>
...
/* Estilo para filas de peticiones ya finalizadas */
